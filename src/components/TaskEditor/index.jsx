...
export const TaskEditor = ({ column, isQuickAdd, isEdit, task, closeOverlay }) => {
  ...
  // New state for title
  const [title, setTitle] = useState(isEdit && task.title); // Assuming task has a title field
  ...
  const addTaskToFirestore = async (event) => {
    ...
    try {
      ...
      await addDoc(collection(db, "user", `${currentUser && currentUser.id}/tasks`), {
        ...
        date: schedule.date,
        name: taskName,
        title: title, // Include title in the task data
        taskId: taskId,
        completed: false,
        boardStatus: "TODO",
  ...
  const updateTaskInFirestore = async (e) => {
    ...
    taskDocs.forEach(async (taskDoc) => {
      await updateDoc(taskDoc.ref, {
        name: taskName,
        title: title, // Update title in Firestore
        date: schedule.date,
        ...
        >
          <div className={`add-task__container ${isQuickAdd ? " quick-add__container" : ""}`}>
            ...
            <input
              ...
              type="text"
              maxLength="30"
              placeholder="Task Title" // Placeholder for the title input
            />
